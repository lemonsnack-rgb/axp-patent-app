...
import { docKindLabel, pubSeriesLabel, pubValues, isBulletinDate, stripCountry, isDeletedClaim, familyCounts, rightChangeCell } from './patentDetailRules.ts';
...
test('문헌종류: 공개계 코드도 라벨로 치환한다(적재 시 코드값 확인 필요)', () => {
  assert.equal(docKindLabel('A'), '공개특허공보');
  assert.equal(docKindLabel('U'), '공개실용신안공보');
});

test('문헌종류: 매핑에 없는 코드는 추정하지 않고 원값을 둔다', () => {
  assert.equal(docKindLabel('B9'), 'B9');
...
// ── 공개/국제공개/공고 값 배치 (지시서 §A-1) ──────────────────────────────
// 항목(라벨)은 고정이다. 값을 어느 칸에 넣을지만 판정한다.

test('공개계: 공개번호+공개일, 공고일은 publication_date', () => {
  // KR 10-2620084 B1
  assert.deepEqual(pubValues('10-2018-0013604', '2024-01-02', '2018-02-07', '2023-12-27'), {
...
test('국제공개(WO): publication_date 는 국제공개일, open_date 는 국내 공개일', () => {
  // KR 10-2635013 B1 — 등록 2024-02-05 인데 publication_date 는 2011-11-24
  assert.deepEqual(pubValues('WO 2011/146808', '2011-11-24', '2022-07-26', '2024-02-05'), {
    공개번호: '', 공개일: '2022-07-26',
    국제공개번호: 'WO 2011/146808', 국제공개일: '2011-11-24',
    공고번호: '', 공고일: '',                       // 공고일 데이터가 없다
  });
});

test('공고일이 아닌 publication_date 는 공개일 칸으로 간다', () => {
  // KR 20-0497874 Y1 — open_date 결손, publication_date 가 공개일
  const v = pubValues('20-2023-0000689', '2023-04-05', undefined, '2024-03-18');
  assert.equal(v.공개일, '2023-04-05');
  assert.equal(v.공고일, '');
});

test('공고계: 공고번호와 공고일이 함께, 공개일은 open_date', () => {
  // KR 10-2649868 B1
  const v = pubValues('10-2649868', '2024-03-20', '2023-03-07', '2024-03-18');
  assert.equal(v.공고번호, '10-2649868');
  assert.equal(v.공고일, '2024-03-20');
  assert.equal(v.공개일, '2023-03-07');
  assert.equal(v.공개번호, '');
});

test('번호 없음: 공고일만 채워진다', () => {
  // KR 10-2618432 B1 — 공개 없이 등록
  const v = pubValues('', '2024-01-02', undefined, '2023-12-21');
  assert.equal(v.공고일, '2024-01-02');
  assert.equal(v.공개번호, '');
  assert.equal(v.공개일, '');
});

test('미등록 문헌: 등록일 자리표시자에서 공고일로 새지 않는다', () => {
  const v = pubValues('US 2025/0098006 A1', '2025-10-12', '2024-03-09', '-');
  assert.equal(v.공고일, '');            // '2025-10-12' >= '-' 로 통과하면 안 된다
  assert.equal(v.공개일, '2024-03-09');
});
