...
test('applyScope: 선두 자유검색어에 범위 인덱스를 적용', () => {
  assert.equal(applyScope('하이브리드 자동차', 'KEY'), 'KEY:(하이브리드 자동차)');
});
test('applyScope: 이미 필드 지정된 식은 건드리지 않는다', () => {
  assert.equal(applyScope('TI:(엔진)', 'KEY'), 'TI:(엔진)');
});
test('applyScope: 선두 자유검색어 뒤에 AND 절이 있으면 선두만 감싼다', () => {
  assert.equal(applyScope('하이브리드 AND TI:(엔진)', 'TAC'), 'TAC:(하이브리드) AND TI:(엔진)');
});
