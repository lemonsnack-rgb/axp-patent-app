...
- **검증 결과(역방향)**: UI 표시 항목 중 수집 근거가 없는 항목 **0건**. 계산·파싱 파생만 컬럼 비대상(아래 `⚠파생`). 과거 **특허평가·소송/분쟁** 항목은 명세서에 컬럼이 없어 **UI에서 삭제 완료**.
...
- **대조 방식(2026-07-28)**: 화면에 부착한 수집 컬럼 표기의 컬럼·테이블 토큰 **112개**를 원본 명세서 PDF 추출 텍스트와 기계 대조했다. **명세서에 없는 토큰 0건.** 단 이 대조는 **토큰 존재 여부**까지이며, 컬럼이 그 테이블에 속하는지는 도면·특허고객번호 등 개별 확인한 항목에 한한다.
...
| 단위 접미 | 청구항 수 → `N개`, 도면 수·패밀리 → `N건` | `10개` / `5건` |
...
| 도면 수 | `5건` | `KPA_BIBLIOGRAPHIC.drawing_count` |
...
| 우선권 주장(목록) | A: 블록 미표시 · **C**: `EP 20000068 · 2012-05-05` · **D**: `US 17000521 · 2021-02-02` | `priority`(priority_application_country_code · number · date) · JP `PRIR` · 조건 |
...
| UI 명칭 | 예시 (A) | 수집 필드 |
|---|---|---|
| 요약 | `라이다 포인트 클라우드를 딥러닝으로 처리하여 보행자·차량·장애물을 실시간 감지·분류하는 장치 및 방법. 본 발명에 따른 객체 감지 장치는 …` (약 300자, 회색 박스에 전문) | `abstract.abstract` (JP/CN 영문요약 `e_abstract`) |
| 상세설명 — 발명의 구체적인 내용 | `[기술분야] 【0001】 본 발명은 자율주행 차량용 라이다 기반 객체 감지에 관한 것으로 …` (문단 번호 【0001】~【0023】, 개행 유지) | `specification.specification` (KIPRIS는 전문 PDF URL) · `⚠목업` |
| 상세설명 — 기술분야 · 배경기술 | `본 발명은 자율주행 차량용 라이다 기반 객체 감지에 관한 것으로, 해당 기술분야의 장치 및 방법에 관한 것이다.` / `종래 기술은 정확도와 견고성 측면에서 한계가 있었으며 …` | `⚠파생` — `specification` 파싱. **현재 화면은 고정 문장(데이터 미연동)** |
| 상세설명 — 해결하려는 과제 · 과제의 해결 수단 · 발명의 효과 | `주변 환경 인식 정확도와 실시간 처리 속도를 동시에 향상한다.` / `지면 분리와 딥러닝 인식을 2단계로 처리한다.` / `야간·악천후에서 카메라 단독 대비 18% 인식률 향상.` | `⚠파생` — `specification` 파싱(구조 분해 규칙 정의 필요) |
| 상세설명 — 도면의 설명 | `FIG 1 객체 감지 장치의 전체 구성도` … `FIG 5 성능 비교 그래프` | `⚠파생` — `specification` 파싱 |
| 청구범위(전체) | 10개 항 — 독립항 2개(제1·8항)·종속항 8개 | `claim.claim` · `⚠목업` |
| 독립항/종속항 구조 · 인용 관계 | `독립항 — 제1항` / `종속항 (제2항 → 제1항 인용)` | `⚠파생` 구조 분해 (대표청구항 번호는 `KPA_BIBLIOGRAPHIC.representation_claim_number`) |
...
> 표시 특이사항: `기술분야`·`배경기술` 2개 하위섹션은 **수집 데이터가 아니라 화면에 하드코딩된 문장**이다(기술분야만 발명의 명칭을 끼워 넣음). 나머지 4개는 데이터 기반. 연동 시 `specification` 파싱 결과로 6개 모두를 채워야 한다.
...
| 패밀리 문헌(국가·번호·일자·명칭) | A: `KR  KR 10-2026-1000000 A  2026-01-03  자율주행 차량용 라이다 기반 객체 감지` (1건) · **B**: 4건(CN·EP·KR·US) | `family`(family_country_code · family_literature_number · application_date · invention_title) |
...
| UI 명칭 | 예시 (A) | 수집 필드 |
|---|---|---|
| 인용 / 피인용 — 특허 정보 | 헤더 `인용 (4건)` / `피인용 (2건)`, 항목 `KR 10-2023-1004513 · 자율주행 차량용 라이다 기반 객체 감지 관련 선행기술` | KR `KPA_CITATION`(citation_literature_number · country_code) · JP/US `CTLTR` · `⚠목업` |
| 인용 / 피인용 — 비특허(논문) 정보 | `[NPL] LiDAR-Based Object Detection for Autonomous Driving: A Review, IEEE/Elsevier, 2025` | US `CTLTR_ETC.other_citations` · `PRIOR_TECHNOLOGY_DOCUMENT.non_patent_reference_text` |
| 선행기술문헌 | `선행기술문헌 (1건)` · `KR KR 10-2023-1000000 A` | `PRIOR_TECHNOLOGY_DOCUMENT`(prior_technology_document_number · country) · 조건 |
...
| 권리변동 이력 | **B**: `2018-04-04 │ 百度在线网络技术有限公司 → OO기술지주(주) │ 권리 양도` | `RIGHT_CHANGE`(change_type · name · change_date) · 조건 |
| 권리이전 이력 | **B**: `2018-04-04 │ 권리이전등록신청서 (百度在线网络技术有限公司 → OO기술지주(주)) │ CN 1010299973 B` | `RIGHT_TRANSFER`(document_name · change_before/after_content · registration_number/date) · 조건 |
| 행정처리(수발신) 이력 | A: `2025-01-01 출원서 수리` · **B**: 3건(출원서 수리 / 의견제출통지서 발송 / 등록결정서 발송) | `ADMINISTRATIVE_PROCESS`(receipt_send_document_name · date · proc_status) · 조건 |
| 국가 R&D 정보 | A: `자율주행 차량용 라이다 기반 객체 감지 원천기술 개발 (2025-000000)` / `산업통상자원부 · 차세대 핵심기술개발사업 · 현대자동차주식회사 · 2024.03 ~ 2026.02` | `RND`(rnd_task_name · number · department · managing_institute · duration) · 조건(KR) |
| 표준특허 | A: `3GPP` / `TS 38.300` / `자율주행 차량용 라이다 기반 객체 감지 표준` / `현대자동차주식회사` / `2026-01-01` | `STANDARD`(standardization_organization · standard_numbers · technology_name · declarants · date) · 조건 |
| 심판 정보 | A: `무효심판 계속 중 (2026당0000)` · B~E: `심판 없음` → 블록 미표시 | `custom.has_trial` |
| 관련출원(US) | **D**: `US 9000013 │ 2021-02-02 │ Continuation │ Granted` | `REL_APPL`(registration_number · date · classification · status) · 조건(US) |
...
| UI 명칭 | 예시 (A) | 수집 필드 |
|---|---|---|
| 도면 이미지 | `도면 (5)` — `FIG 1`~`FIG 5`, 현재 이미지는 임시 생성 도식(블록도·흐름도·적층구조·그래프 4종 순환) | `patent_image`(scrape_site · key_name → S3 경로, 둘 다 NOT NULL) · `custom.drawing_url`(KR·CN) · `custom(JP).figures` · `⚠목업` |
| 대표도면 | `대표` 배지 + `FIG 1 객체 감지 장치의 전체 구성도` | `custom.primary_drawing_url`(KR·CN) · `⚠목업` |
| 도면 라벨 | `FIG 1` … `FIG 5` | `custom(JP).figures`(목록 내 항목) · 그 외 국가는 `patent_image.key_name` 순서 파생 · 조건 |
| 도면 주요 부분에 대한 부호의 설명 | `100 객체 감지 장치` / `110 데이터 수집부` / `120 전처리부` / `130 인식부` | `⚠파생` — `specification` 파싱(전용 컬럼 없음) |
| (UI 미노출) 도면 영역 수 | — | `KPA_BIBLIOGRAPHIC.drawing_area_count` |
| (UI 미노출) 대표도면 출처 상태 | — | `KPA_BIBLIOGRAPHIC.representation_image_source_status_code` |
...
도면 (5)                                       ← 도면 건수
┌──────────────────────────────────────┐
│ [대표] FIG 1  객체 감지 장치의 전체 구성도  ⤢ 확대 │  ← 첫 도면 선택 시에만 [대표] 배지
│           (선택 도면 크게 — 클릭 시 확대 모달)     │
└──────────────────────────────────────┘
대표도면
 ┌─────────┐
...
> 표시 특이사항: 확대 모달은 휠 확대/축소(50~500%)·드래그 이동·`←/→` 도면 이동·`+/-/0` 줌·`Esc` 닫기를 지원하며, 하단에 전체 도면 썸네일 스트립과 `n / 5` 카운터가 표시된다. 도면이 없으면 패널 전체가 `도면 없음`(아이콘+문구)으로 대체된다.
