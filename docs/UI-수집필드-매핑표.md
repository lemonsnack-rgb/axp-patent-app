...
- **검증 결과(역방향)**: 화면 표시 조각 중 대응 컬럼이 없는 것은 **문서에 판정을 표기**하고(§12-1), UI 제거는 **명시적 확인을 받은 경우에만** 한다. 현재 제거 상태: 인용문헌 명칭 · (과거) 특허평가 · 소송/분쟁.
- **파싱 반입 확정(2026-07-31)**: 상세설명 하위섹션(기술분야·배경기술·해결하려는 과제·과제의 해결 수단·발명의 효과) · 도면의 설명(캡션) · 도면 부호의 설명 · 독립항/종속항 구조는 `specification`·`claim` 원문을 **파싱해 반입**한다. 「미수집」이 아니라 `PARSE(...)` 파생이며, **연동 시 파싱 규칙(섹션 표제 사전)을 정의**해야 한다.
...
- **수집 컬럼 열은 「조합식」으로 적는다** — 화면 값이 컬럼 하나가 아니면 **어떻게 조합·가공하는지**를 식으로 표기한다. 표기 규약:
  ...
  | `PARSE(x, 규칙)` | 원문 텍스트 파싱으로 반입(전용 컬럼 없음 · 파싱 규칙 정의 필요) | `PARSE(specification, '기술분야' 섹션)` |
...
| 상세설명 — 기술분야 | `techField` | `본 발명은 자율주행 차량용 라이다 기반 객체 감지에 관한 것으로 …` | `PARSE(specification, '기술분야' 섹션)` — **파싱 반입 확정**(2026-07-31) |
| 상세설명 — 배경기술 | `background` | `최근 라이다 및 자율주행 기술의 수요가 급격히 증가함에 따라 …` | `PARSE(specification, '배경기술' 섹션)` |
| 상세설명 — 해결하려는 과제 | `aiPurpose` | `주변 환경 인식 정확도와 실시간 처리 속도를 동시에 향상한다.` | `PARSE(specification, '해결하려는 과제' 섹션)` |
| 상세설명 — 과제의 해결 수단 | `aiSolution` | `지면 분리와 딥러닝 인식을 2단계로 처리한다.` | `PARSE(specification, '과제의 해결 수단' 섹션)` |
| 상세설명 — 발명의 효과 | `aiEffect` | `야간·악천후에서 카메라 단독 대비 18% 인식률 향상.` | `PARSE(specification, '발명의 효과' 섹션)` |
| 상세설명 — 도면의 설명 | `figures[].desc` | `FIG 1 객체 감지 장치의 전체 구성도` … `FIG 6 변형 실시예의 구성도` | `PARSE(specification, '도면의 간단한 설명' 섹션)` → `[N행] 'FIG ' + (순번+1) + ' ' + 설명` |
...
**목업 렌더 — 상세설명 (표본 A)** · 회색 박스 안 하위섹션 + 원문

```
기술분야            ← PARSE(specification, '기술분야')
 본 발명은 자율주행 차량용 라이다 기반 객체 감지에 관한 것으로, 라이다 센서로부터 …
배경기술            ← PARSE(specification, '배경기술')
 최근 라이다 및 자율주행 기술의 수요가 급격히 증가함에 따라 …
해결하려는 과제      ← 값 있을 때만
...
도면의 설명          ← 도면 있을 때만
 ...
 …
 FIG 6  변형 실시예의 구성도
발명의 구체적인 내용  ← specification 원문 전문
 [기술분야]
...
> **파싱 반입 확정(2026-07-31)**: 하위섹션은 `specification` 원문의 섹션 표제(`[기술분야]`·`[배경기술]`·`[과제의 해결 수단]`·`[발명의 효과]`·`[도면의 간단한 설명]` 등)를 기준으로 분해해 채운다. 표제 표기가 소스·국가별로 다를 수 있어 **파싱 규칙(표제 사전)** 을 연동 시 정의해야 한다.
...
| 도면 캡션(도면의 설명) | `figures[].desc` | `객체 감지 장치의 전체 구성도` | `PARSE(specification, '도면의 간단한 설명' 섹션)` — **파싱 반입 확정**(2026-07-31) |
| 도면 주요 부분에 대한 부호의 설명 | `refSigns` | `100 객체 감지 장치` / `110 데이터 수집부` / `120 전처리부` / `130 인식부` | `PARSE(specification, '부호의 설명' 섹션)` → `[N행] 부호 + 명칭` — **파싱 반입 확정**(2026-07-31) |
...
도면 (6)                                       ← 이미지 건수
┌──────────────────────────────────────┐
│ [대표] FIG 1  객체 감지 장치의 전체 구성도  ⤢ 확대 │  ← 라벨(순번) + 캡션(파싱)
└──────────────────────────────────────┘
대표도면   [대표] 썸네일 FIG 1                  ← 폭 1/2, 선택 시 파란 링
그 외 도면 (5)   [FIG 2] [FIG 3] [FIG 4] [FIG 5] [FIG 6]

도면 주요 부분에 대한 부호의 설명               ← PARSE(specification, '부호의 설명')
 100   객체 감지 장치
 110   데이터 수집부
 120   전처리부
 130   인식부
...
> 확대 모달: 휠 확대/축소(50~500%)·드래그 이동·`←/→` 도면 이동·`+/-/0` 줌·`Esc` 닫기 + 하단 썸네일 스트립·`n / 6` 카운터 + 부호의 설명. 도면이 없으면 패널 전체가 `도면 없음`으로 대체된다.

---

## 10. 요약 통계 및 잔여 과제

- **표시 항목 근거**: 전 항목이 (a) 수집 컬럼 직결, (b) 계산 `⚠파생`(존속만료 · 타임라인 · 도면 라벨 · 패밀리 국가 집계), 또는 (c) **`PARSE(...)` 파싱 반입**(상세설명 하위섹션 · 도면의 설명 · 부호의 설명 · 청구항 독립/종속 구조)이다. 대응 근거가 없어 제거한 항목만 §12-1.
- **연동 시 결정/작업**
  ...
  - **P1 · 파싱 규칙 정의**: 상세설명 하위섹션·도면의 설명·부호의 설명·청구항 독립/종속 구조가 모두 파싱 반입이므로, **섹션 표제 사전과 청구항 패턴**(`제N항`·`제M항에 있어서`)을 국가·소스별로 확정해야 한다.
...
| ↳ | `FIG 1` | `⚠파생` — 이미지 순번(화면 생성) = `'FIG ' + (순번+1)` |
| ↳ 캡션 | `객체 감지 장치의 전체 구성도` | `PARSE(specification, '도면의 간단한 설명')` |
| ↳ 부호의 설명 | `110 데이터 수집부` | `PARSE(specification, '부호의 설명')` → `[N행] 부호 + 명칭` |
| 상세설명 하위섹션 | 기술분야 / 배경기술 / 과제 / 해결수단 / 효과 | `PARSE(specification, 각 섹션 표제)` — 파싱 반입 확정 |
...
### 12-1. 미수집으로 UI에서 제거한 항목 (제거는 명시적 확인을 받은 것만)
...
> **2026-07-31 정정**: 상세설명 하위섹션 6개 · 도면 캡션 · 도면 부호의 설명은 한때 제거했으나, **`specification` 파싱으로 반입하기로 확정**되어 **UI를 복원**했다(§4·§9). 독립항/종속항 구조도 같은 방식(`claim` 파싱)으로 유지한다. 즉 이 4종은 「미수집」이 아니라 **`PARSE(...)` 파생**이다.
